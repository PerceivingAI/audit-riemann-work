...
* **Primary Source Found**:
  ...
  - Classical Identity: $\int_{-1}^1 rac{P_n(x)-P_n(y)}{|x-y|} dy = 2 H_n P_n(x)$, where $H_n = \sum_{k=1}^n 1/k$.
...
* **Findings**:
  ...
  - Componentwise separation of the tail Gram form into Archimedean singular form ($G_V$), prime translation form ($G_2$), and residual kernel form ($G_R$) combined with the factor-3 Cauchy-Schwarz estimate $A_N - rac{3}{\mu_N}(G_V + G_2 + G_R) > 0$ is unique to `riemann-conjecture`.
